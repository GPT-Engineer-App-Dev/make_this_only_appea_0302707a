...
const Index = () => {
  ...
  const [showWarehousingText, setShowWarehousingText] = useState(false);

  useEffect(() => {
    const initialTimer = setTimeout(() => {
      setShowInitialText(false);
      setShowWarehousingText(true);
    }, 5000);
    const warehousingTimer = setTimeout(() => {
      if (showWarehousingText) {
        setShowWarehousingText(false);
      }
    }, 10000);

    return () => {
      clearTimeout(initialTimer);
      clearTimeout(warehousingTimer);
    };

    // Removed the unnecessary return statement that caused the error
  }, []);
